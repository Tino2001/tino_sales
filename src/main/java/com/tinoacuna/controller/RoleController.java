...
import com.tinoacuna.dto.RoleDTO;
...
public class RoleController {
    ...
    public ResponseEntity<List<RoleDTO>> readAll() throws Exception{
        List<RoleDTO> list = service.readAll().stream().map(cat -> mapper.map(cat, RoleDTO.class)).collect(Collectors.toList());
    ...
    public ResponseEntity<RoleDTO> update(@Valid @RequestBody RoleDTO dto) throws Exception{
        ...
        Role cat = service.update(mapper.map(dto, Role.class));
        return new ResponseEntity<>(mapper.map(cat, RoleDTO.class), HttpStatus.OK);
    }
